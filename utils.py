...
def process_video_task_with_transcoder(video_url, user_id, task_id, whisper_language, max_segment_mb, webhook_url, prompt):
    ...
    try:
        ...
        for batch_idx, chunk_path in enumerate(audio_chunks):
            ...
            logger.info(f"📝 批次 {batch_count} 轉錄完成。片段時長: {chunk_duration:.2f}s。累計 offset: {total_duration_offset:.2f}s")
        if final_srt_parts:
            ...
            payload = {"任務狀態": "成功", "user_id": user_id, "task_id": task_id, "video_url": video_url, "whisper_language": whisper_language, "srt_url": srt_url, "影片原始大小MB": total_mb, "音檔批次數": len(audio_chunks), "總時長秒": total_duration_offset, "轉換方式": "Google Transcoder API", "程式版本": VERSION}
        ...
        payload = {"任務狀態": f"失敗: {str(e)}", "user_id": user_id, "task_id": task_id, "video_url": video_url, "whisper_language": whisper_language, "srt_url": "", "程式版本": VERSION}
        try:
            requests.post(webhook_url, json=payload, timeout=10)
        except:
            pass
